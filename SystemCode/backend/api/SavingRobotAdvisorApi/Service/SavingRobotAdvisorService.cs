...
namespace SavingRobotAdvisorApi.Service
{
    ...
    {
        public List<OptimalResult> GetOptimalResult(decimal monthlyIncome, decimal savingBalance, MonthlySpending monthlySpending)
        {
            ...
            foreach (Bank bankName in Enum.GetValues(typeof(Bank)))
            {
                ...
                {
                    results.Add(CalculationFactory.GetCalculationResult(bankName, monthlyIncome, savingBalance, monthlySpending));
                }
            }
        ...
        public List<OptimalResult> GetOptimalResult(RobotAdvisorRequest request)
        {
            List<OptimalResult> optimalResults = new List<OptimalResult>();
            List<CalculationResult> results = new List<CalculationResult>();
            ...
            foreach (Bank bankName in Enum.GetValues(typeof(Bank)))
            {
                if (bankName != Bank.Unkonwn)
                {
                   results.Add(CalculationFactory.GetCalculationResult(bankName, request.Income, request.Balance, request.MonthlySpending));
                }
            }
